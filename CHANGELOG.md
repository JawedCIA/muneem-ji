...
### Added
- **Feature toggles + shop-type setup** — six per-shop toggles (`feature.serials`, `feature.batches`, `feature.banking`, `feature.recurring`, `feature.pos`, `feature.quotations`) hide / show modules in the sidebar. Reachable in Settings → Business Profile → Features. The first-run setup wizard now asks "what kind of shop is this?" with nine archetype cards (General store, Pharmacy, Electronics, Restaurant, Service business, Wholesale, Jewellery, Auto / repair, Other) — picking one flips the toggles in one click; users can still tweak any individual toggle later. Migration `012_feature_flags.sql` seeds defaults and *auto-detects* on existing installs (if any serials / batches / recurring / bank-accounts data is present, the corresponding toggle stays ON). Fresh installs default to POS + Quotations + Banking visible. Hiding a module never deletes its data — flip the toggle back on and everything reappears. Test suite is now **166 / 166**.
- **Batch + expiry tracking** — products can be flagged "Has batch / expiry" with an optional default shelf-life (days). When set, every invoice line for that product requires `batch_no` and `exp_date`; `mfg_date` is optional but expiry is checked to be ≥ mfg date. Server validation runs on both sales and purchases (so pharmacy can capture batch on incoming stock too). Optional `shelf_life_days` on the product auto-suggests expiry from the entered manufacturing date. New **Batches** page in the sidebar: search by batch / product / invoice, filter by status (Active / Expiring within 30d / Expired / No expiry) and by movement type (Sale / Purchase), with a stats header. Batch + Mfg + Exp render under the item name in the InvoicePreview and the generated PDF (works in both 7-col GST and 5-col GST-off layouts). Migration `011_batch_expiry.sql` adds `products.has_batch` + `products.shelf_life_days` and `invoice_items.batch_no` / `mfg_date` / `exp_date` with indexes on `exp_date` and `batch_no`. 11 new e2e scenarios cover validation, persistence, expiry buckets, type filter, and the line-item clear-on-product-change case. Test suite is now **164 / 164**.
- **Serial / IMEI tracking + warranty register** — products can be flagged "Has serial numbers" with a default warranty in months. Sales of those products require one serial per unit (validated server-side: count must equal qty, no duplicates within an invoice). Warranty period rolls forward from invoice date and is stored as `warranty_until` for fast lookup. New **Serials** page in the sidebar: search any serial / IMEI / customer / invoice number, filter by warranty status (active / expiring within 30 days / expired / no warranty), see who bought what when. Lookup API `/api/serials/lookup/:serial` is case-insensitive — supports walk-in service intake ("scan IMEI → find original sale"). Cascading delete on invoices keeps the register consistent. Migration `010_serials_warranty.sql` adds `products.has_serial`, `products.warranty_months`, and the new `item_serials` table with indexes on serial / product / invoice / party / warranty_until. Test suite is now **153 / 153**.
- **GST on/off toggle** — `gstEnabled` setting (default ON for backwards compatibility). Asked during the first-run setup wizard and re-flippable any time under Settings → Business Profile. When OFF: GSTIN field hides, invoice form / preview / PDF / POS drop the HSN, Tax%, CGST/SGST/IGST and "Tax Invoice" header (becomes plain "Invoice"); Reports sidebar hides GST Summary, GSTR-1 and GSTR-3B tabs; new line items default to 0% tax. The change applies live — no restart, no schema impact on existing invoices. Public share-link viewer respects the toggle via the branding endpoint. Migration `009_gst_optional.sql` seeds the setting; e2e suite is now **143 / 143**.
